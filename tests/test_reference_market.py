...
from nobitex_bot.data.reference_market import ReferenceMarketCollector, nobitex_symbol_to_binance_symbol
from nobitex_bot.data.storage import Storage
from nobitex_bot.exchange.models import Candle


def test_nobitex_symbol_to_binance_symbol_maps_simple_markets():
    assert nobitex_symbol_to_binance_symbol("BTCIRT") == "BTCUSDT"
    assert nobitex_symbol_to_binance_symbol("ETHUSDT") == "ETHUSDT"


def test_nobitex_symbol_to_binance_symbol_skips_scaled_markets():
    assert nobitex_symbol_to_binance_symbol("1M_BTTIRT") is None


def test_nobitex_symbol_to_binance_symbol_skips_invalid_format():
    assert nobitex_symbol_to_binance_symbol("garbage") is None
...
def test_collect_saves_candles_for_mappable_symbols(tmp_path):
    ...
    client = MagicMock()
    client.get_klines.return_value = [_candle()]
    collector = ReferenceMarketCollector(storage=storage, client=client)

    saved = collector.collect(["BTCIRT"], resolution="60")

    assert saved == 1
    client.get_klines.assert_called_once_with("BTCUSDT", "60", limit=200)
    stored = storage.get_reference_candles("binance", "BTCUSDT", "60")
    assert len(stored) == 1
    storage.close()
...
def test_collect_continues_after_one_symbol_raises(tmp_path):
    ...
    storage = Storage(tmp_path / "test.sqlite")
    client = MagicMock()
    client.get_klines.side_effect = [RuntimeError("boom"), [_candle()]]
    collector = ReferenceMarketCollector(storage=storage, client=client)
